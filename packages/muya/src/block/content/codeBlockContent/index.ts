 ...
 * a byte-identical string. An IME composes against a live text node, so a
 * rebuild between two characters costs it its anchor and the input method
 * garbles everything after the first one (#4851). Plain CJK never changes the
 * rendered output, so comparing first keeps that node alive — the same reason
 * `Format.inputHandler` skips `update()` when `checkNeedRender` reports no
 * token change.
 */
