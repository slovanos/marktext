...
// #4851 — Japanese IME input breaks inside a code block: the first kana
// commits, the second and later ones come out garbled ("ｎあ" / repeated kana).
...
// byte-identical. An IME composes against a live text node, so committing one
// character replaced the node the next composition was about to start from.
//
// The glyph-level corruption is IME-internal and not reproducible headlessly
// (`e2e/tests/typing/ime-codeblock-4851.spec.ts` drives a real Chromium
// composition over CDP and pins node identity there). What these tests pin is
// the property that makes it impossible: a render whose output has not changed
// must leave the existing DOM nodes in place.
...
describe('codeBlockContent render idempotence (#4851)', () => {
